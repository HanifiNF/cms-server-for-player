...
class MediaLibraryController extends BaseController
{
    ...
    public function show(string $publicId): string|RedirectResponse
    {
        ...
        $assignments = $db->table('device_assets da')
            ->select('da.status, da.last_reported_at, d.public_id AS device_public_id, d.name AS device_name, d.status AS device_status, l.public_id AS location_public_id, l.name AS location_name')
        ...
        if ($isAdmin && $asset->status === 'active') {
            $activeLocations = array_map(static fn ($location): int => (int) $location->id, (new LocationModel())->where('status', 'active')->findAll());
            $assignedIds = array_map(static fn (array $row): string => (string) $row['device_public_id'], $assignments);
            foreach ((new DeviceModel())->where('status', 'active')->orderBy('location')->orderBy('name')->findAll() as $device) {
                if ($device->location_id !== null && ! in_array((int) $device->location_id, $activeLocations, true)) continue;
                if (in_array((string) $device->public_id, $assignedIds, true)) continue;
                $devices[] = $device;
            }
        }
        return view('web/media_library_detail', [
            ...
            'assignments' => $assignments, 'devices' => $devices, 'schedules' => $schedules,
