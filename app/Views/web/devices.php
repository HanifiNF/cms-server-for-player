...
<section class="card">
  ...
  <?php if ($devices === []): ?><div class="empty">No Player records yet.</div><?php else: ?><div class="device-grid"><?php foreach ($devices as $item): $device = $item['entity']; ?>
    <article class="device-card"><div class="device-title"><span class="device-icon">▶</span><div><strong><?= esc($device->name) ?></strong><small><?= esc($device->location ?: 'No location') ?></small></div><span class="badge <?= esc($item['connection']) ?>"><?= esc(strtoupper($item['connection'])) ?></span></div>
      <dl><div><dt>Device ID</dt><dd title="<?= esc($device->public_id) ?>"><?= esc($device->public_id) ?></dd></div><div><dt>Assigned to</dt><dd><?= esc($item['assignedName']) ?></dd></div><div><dt>Version</dt><dd><?= esc($device->app_version ?: 'Not paired') ?></dd></div><div><dt>Media protection</dt><dd><?= $device->ldg_version === 'ldg-v1' ? 'LDG v1 ready' : 'Player update required' ?></dd></div><div><dt>Assets</dt><dd><?= (int) $item['assetCount'] ?> · revision <?= (int) $device->inventory_revision ?></dd></div><div><dt>Last seen</dt><dd><?= $device->last_seen_at ? esc($device->last_seen_at->format('Y-m-d H:i:s')) . ' UTC' : 'Never' ?></dd></div></dl>
